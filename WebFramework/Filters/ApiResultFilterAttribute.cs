    ...
    public class ApiResultFilterAttribute : ActionFilterAttribute
    {
        public override void OnResultExecuting(ResultExecutingContext context)
        {
            ...
            else if (context.Result is BadRequestObjectResult badRequestObjectResult)
            {
                ...
                if (badRequestObjectResult.Value is SerializableError serializableError)
                {
                    var errorMessages = serializableError.SelectMany(p => (string[])p.Value).Distinct();
                    message = string.Join(" | ", errorMessages);
                }else if (badRequestObjectResult.Value is ValidationProblemDetails validationProblemDetails)
                {
